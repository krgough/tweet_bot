...
def set_shutdown_mode(addr=DEV_ADDR):
    ...
    Device draw 3uA in shutdown.
    Serial IF still operates.
    """
    cfg = read_config_reg()
...
def set_one_shot(addr=DEV_ADDR):
    ...
    Device returns to shutdown after completion
    """
    cfg = read_config_reg()
...
def convert_raw_temp_to_float(raw_temp, bits=12):
    """Convert the raw temperature

    LSB is temp in degree resolution 1'C in 2s complement
