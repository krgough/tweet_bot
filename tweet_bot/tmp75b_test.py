...
def read_config_reg(addr=DEV_ADDR):
   """  Config register is 0x01
        ...
        BIT 14 = CR --- Conversion Rate 0h=37Hz (default), 1h=18Hz, 2h=9Hz, 3h=4Hz
        BIT 13 = CR /
        BIT 12 = FQ --- Fault queue to trigger ALERT pin. 0h=1fault, 1h=2faults, 2h=4fault, 3h=6faults
        BIT 11 = FQ /   Numer of readings that exceed T_high or T_low before ALERT is set
        BIT 10 = Alert Polarity.  0h = active low, 1h active high
        BIT  9 = Thermostat Mode. 0h=Alert is comparator mode, 1h=Alert is interrupt mode
...
def convert_raw_temp_to_float(raw_temp, bits=12):
    ...
    LOGGER.debug(f"Raw={raw_temp:016b} Temp={temp_float}'C")
...
def read_temperature(addr=DEV_ADDR, one_shot=True):
    ...
    if one_shot:
        ...
        time.sleep(0.1)
    raw_temp = BUS.read_word_data(addr, 0x00)
